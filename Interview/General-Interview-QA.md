...
# SECTION 9: Terraform & IaC

---

### Q: Tell me about your use cases in Terraform and have you written Terraform code?

**Project Reference:** P2 (3-Tier AWS), P4 (Landing Zone), P8 (Multi-Region DR)

**Answer:**

> "Yes, extensively. Terraform is my primary IaC tool. Key use cases:
>
> 1. **Reusable module library** — Built modules for VPC, EKS, RDS, IAM that are used across all environments and accounts. One module, parameterized per environment via tfvars.
>
> 2. **Multi-account Landing Zone (P4)** — 15 AWS accounts, 5 OUs provisioned entirely with Terraform. SCPs, IAM Identity Center, OIDC federation — all codified.
>
> 3. **3-Tier production architecture (P2)** — VPC (3 AZs), ALB + WAF + CloudFront, EC2 ASG, Aurora Multi-AZ. Handles 5000 req/s.
>
> 4. **Multi-region DR (P8)** — Route53 failover, Aurora Global Database, S3 cross-region replication — all Terraform.
>
> **CI/CD for Terraform:** Every PR runs `terraform plan` + tfsec + Checkov + Infracost. Merged to main → `terraform apply` via pipeline with approval gate. No manual applies, no local state."

---

### Q: What do you understand by drift in infrastructure as code?

**Project Reference:** P2, P4 (Terraform with CI/CD)

**Answer:**

> "Drift = when actual infrastructure doesn't match what's defined in code. Someone made a manual change (ClickOps) that Terraform doesn't know about.
>
> **Example:** Someone manually adds an ingress rule to a Security Group via AWS Console. Terraform state still shows the old rules. Next `terraform plan` will show the drift and try to remove it.
>
> **How we prevent/detect drift:**
> 1. **SCPs** — Block console access for most users. Changes must go through Terraform.
> 2. **Scheduled `terraform plan`** — Runs nightly, alerts if drift detected (plan shows changes when there shouldn't be any).
> 3. **AWS Config rules** — Detect non-compliant resources continuously.
> 4. **ArgoCD (for K8s)** — Drift detection built-in. Manual `kubectl edit` = ArgoCD flags it immediately.
>
> Drift is the enemy of IaC. If you allow manual changes, your Terraform code becomes a lie."

---

### Q: What do you understand by Terraform state file?

**Project Reference:** P2, P4 (Terraform state management)

**Answer:**

> "State file (`terraform.tfstate`) is Terraform's record of what it has deployed. It maps your `.tf` code to real AWS resources (resource IDs, ARNs, IPs).
>
> **Why it matters:**
> - Without state, Terraform doesn't know what exists. It would try to create everything from scratch every time.
> - State enables `plan` — it compares desired (code) vs actual (state) to show what will change.
>
> **How we manage it:**
> - **Remote backend** — S3 bucket + DynamoDB lock table. Never local. Never in Git.
> - **State per environment** — Separate state files for dev/staging/prod. One bad apply doesn't destroy all environments.
> - **State locking** — DynamoDB prevents two engineers from running `apply` simultaneously (race condition).
> - **Never edit state manually** — Use `terraform state mv`, `terraform import`, or `terraform state rm` if needed.
>
> State file contains sensitive data (DB passwords, IPs) — so the S3 bucket is encrypted, versioned, and access-restricted."
...
# SECTION 13: Terraform Advanced

---

### Q: What would be your ideal Terraform directory structure for a multi-account AWS Landing Zone?

**Project Reference:** P4 (Multi-Account Landing Zone — 15 accounts, 5 OUs)

**Answer:**

> "We use a layered structure — separated by account and component:
>
> ```
> terraform/
> ├── modules/                    # Reusable modules (VPC, EKS, IAM, RDS)
> │   ├── vpc/
> │   ├── eks/
> │   ├── iam-role/
> │   └── landing-zone/
> ├── environments/               # Per-account deployments
> │   ├── management/             # Org root, SCPs, IAM Identity Center
> │   │   ├── main.tf
> │   │   ├── variables.tf
> │   │   └── terraform.tfvars
...
> │   ├── staging/
> │   └── dev/
> ├── policies/                   # SCPs, OPA policies
> └── backend.tf                  # S3 + DynamoDB (per-account state)
> ```
>
> **Key design decisions:**
> 1. **Separate state per account** — One bad `terraform destroy` doesn't wipe everything. Each account has its own S3 state file.
> 2. **Modules are shared** — Same VPC module used in all accounts, parameterized via tfvars. DRY.
> 3. **Management account is separate** — SCPs, Organizations, SSO live here. Locked down — only the platform team touches this.
> 4. **Policies as code** — SCPs are Terraform-managed, not ClickOps'd in console.
>
> CI/CD runs `terraform plan` per-account independently. One account's change doesn't block another."

---

### Q: Why would you create your own custom modules instead of using official/third-party ones?

**Project Reference:** P2 (3-Tier — custom module library), P4 (Landing Zone)

**Answer:**

> "Three reasons:
>
> 1. **Security & compliance control** — Third-party modules may include resources configured in ways that violate our security standards (open security groups, unencrypted storage, public subnets by default). With custom modules, every resource follows our CIS/NIST baseline by default. Encryption is on, public access is off — you'd have to explicitly override.
>
> 2. **Opinionated defaults for our org** — Our VPC module always creates 3 AZs, always enables flow logs, always creates private subnets with NAT. A generic community module gives you 50 variables to configure — our module has 5, with sane defaults for our environment. Less room for error.
>
> 3. **No external dependency risk** — Third-party modules can break on updates, get abandoned, or introduce supply chain risk. Our modules are in our private Git repo, versioned, and tested in our CI pipeline. We control the upgrade cycle.
>
> **When I DO use community modules:** For non-critical, well-maintained ones like `terraform-aws-modules/vpc` as a starting point — then fork and customize. Never use them directly in production without review."

---

### Q: Have you worked on dynamic blocks in Terraform? What is their use case?

**Project Reference:** P2 (3-Tier — Security Groups), P4 (Landing Zone — SCPs)

**Answer:**

> "Yes. Dynamic blocks generate **repeated nested blocks** from a variable list — avoiding copy-paste of similar configuration.
>
> **Use case:** Security Group rules. Instead of writing 10 separate `ingress` blocks:
>
> ```hcl
> variable "ingress_rules" {
>   default = [
>     { port = 443, cidr = "10.0.0.0/8", description = "HTTPS internal" },
>     { port = 80,  cidr = "10.0.0.0/8", description = "HTTP internal" },
>     { port = 22,  cidr = "10.1.0.0/16", description = "SSH from bastion" }
>   ]
> }
>
...
>
> **Other use cases:**
> - Multiple SCP statements in an IAM policy document
> - Multiple listeners/rules on an ALB
> - Multiple tags from a map variable
>
> **When NOT to use:** Don't overuse for readability. If you have 2-3 static blocks, hardcode them. Dynamic blocks for 5+ repeated structures or when the list comes from a variable (different per environment)."

---

### Q: What is the difference between terraform taint and terraform destroy?

**Project Reference:** P2 (3-Tier AWS)
...
>
> **Example:**
> - `terraform taint aws_instance.web` → Next apply will destroy and recreate ONLY that instance. Others untouched.
> - `terraform destroy` → Tears down everything Terraform manages.
>
> **Important note:** `terraform taint` is deprecated since Terraform 1.5. The replacement is:
> ```bash
> terraform apply -replace=\"aws_instance.web\"
> ```
> Same effect, cleaner syntax, doesn't modify state file separately.
>
> **When I use replace:** EC2 instance is misbehaving (corrupted disk, stuck userdata), faster to recreate from AMI than debug. Taint/replace → fresh instance from same config."

---

### Q: How do you handle sensitive values like passwords or secrets in Terraform?

**Project Reference:** P4 (Landing Zone), P2 (3-Tier — RDS passwords)

**Answer:**

> "Multiple layers — never hardcode, never in state file unprotected:
>
...
> ```
>
> 2. **Don't pass via tfvars file in Git** — Use environment variables (`TF_VAR_db_password`) injected at CI runtime from Jenkins Credentials or AWS Secrets Manager.
>
> 3. **Use AWS Secrets Manager/SSM directly:**
> ```hcl
...
>
> 4. **State file protection** — State file WILL contain sensitive values (Terraform limitation). So: S3 backend with encryption (KMS), bucket policy restricting access, versioning enabled for recovery.
>
> 5. **Generate passwords in Terraform (not recommended but sometimes needed):**
> ```hcl
...
>
> **Bottom line:** Fetch secrets from Secrets Manager at runtime. Don't store them in code, tfvars, or expose in plan output."
...
### Q: Emergency production fix — manual change vs IaC?

**Project Reference:** P2 (Terraform), P9 (Patching)

**Answer:**

> "**Always IaC — even in emergencies.** Here's why:
>
> If I make a manual change (e.g., add a Security Group rule via console):
> - Next `terraform apply` will REVERT my change (drift)
> - No audit trail of who changed what
> - No peer review
> - Other engineers don't know about it
>
> **What I do instead:**
> 1. Fast PR → merge to main → `terraform apply`. With our pipeline, this takes 5-10 minutes.
> 2. If 5 minutes is too long (true emergency): Make the manual change AND immediately create a PR to codify it. Set a 24-hour SLA to get the IaC caught up. The manual change is temporary — Terraform is the source of truth.
>
> **Never acceptable:** Making a manual change and forgetting about it. That's how drift accumulates and the next apply breaks production.
>
> We have a weekly drift scan (scheduled `terraform plan`). If it shows unexpected changes → someone made a manual fix and didn't codify it. That triggers a conversation."

---

### Q: Use of IaC for long-term, non-updated infrastructure?

**Project Reference:** P4 (Landing Zone — long-lived infra)

**Answer:**

> "Long-lived infra (VPCs, Transit Gateways, IAM policies) that rarely changes still MUST be in IaC. Here's why:
>
...
### Q: Collaboration in IaC — handling remote state and workspaces?

**Project Reference:** P4 (Landing Zone — multi-account), P2 (3-Tier)

**Answer:**

> "Two collaboration patterns we use:
>
> **Remote State (what we use):**
> - S3 backend + DynamoDB state locking
> - State lock prevents two people from running `apply` simultaneously
> - `terraform_remote_state` data source to read outputs across configurations (e.g., networking stack exports VPC ID, app stack reads it)
>
> ```hcl
...
>
> **Workspaces:**
> - We DON'T use Terraform workspaces for environments (confusing, easy to apply to wrong env)
> - Instead: separate directories per environment, separate state files, separate CI pipelines
> - Workspaces are fine for: same config, slightly different parameters (e.g., multi-region same infra)
>
> **Our collaboration model:**
> - PR-based workflow. No one runs `apply` locally.
> - PR triggers `plan` → posted as PR comment → peer reviews the plan → merge → CI runs `apply`
> - State file access restricted via S3 bucket policy (only CI role can write state)"
...
### Q: How will you create a highly available infrastructure using Terraform best practices from scratch?

**Project Reference:** P2 (3-Tier AWS Architecture), P4 (Landing Zone)

**Note:** Different from existing "design HA architecture" (which is AWS services design). This asks about TERRAFORM best practices specifically.

**Answer:**

> "From scratch, here's my approach:
>
> **Terraform Structure:**
> 1. **Remote backend first** — Create S3 bucket + DynamoDB lock table (bootstrap manually or separate Terraform config). Never local state.
> 2. **Module-driven** — Write reusable modules for VPC, compute, database, load balancer. Don't put everything in one `main.tf`.
> 3. **Environment separation** — Separate tfvars per env (`dev.tfvars`, `prod.tfvars`). Same code, different parameters.
>
> **HA Patterns in Terraform:**
> ```hcl
> # Multi-AZ VPC
...
>   max_size         = 20
>   desired_capacity = 3
>   vpc_zone_identifier = module.vpc.private_subnet_ids  # Spans 3 AZs
>   health_check_type = \"ELB\"  # Not just EC2 — checks app health
> }
>
> # Aurora Multi-AZ
...
>
> **Best practices:**
> - `prevent_destroy = true` on critical resources (DB, S3 state bucket)
> - `create_before_destroy` lifecycle for zero-downtime replacements
> - CI/CD pipeline: plan on PR, apply on merge, never `apply` locally
> - Security scanning (tfsec + Checkov) in pipeline
> - Infracost to show cost impact on every PR"
...
# SECTION 18: Terraform Commands & Scenarios

---

### Q: What commands are used in Terraform, and in what scenario do you use terraform destroy?

**Project Reference:** P2 (3-Tier), P4 (Landing Zone)

**Answer:**

> "**Core commands and when to use:**
>
> | Command | Purpose | When |
> |---|---|---|
> | `terraform init` | Initialize providers, modules, backend | First time, or after adding new providers/modules |
> | `terraform plan` | Preview changes (dry run) | Every time before apply. In CI on every PR |
> | `terraform apply` | Execute changes | After plan is reviewed and approved |
> | `terraform destroy` | Remove ALL managed resources | Teardown environments |
> | `terraform validate` | Syntax/config check | In CI as a quick lint step |
> | `terraform fmt` | Format code consistently | Pre-commit hook or CI check |
> | `terraform import` | Bring existing resource into state | When adopting manually-created resources |
> | `terraform state` | Inspect/modify state | Debugging, moving resources, removing from state |
>
> **When I use `terraform destroy`:**
> - Tearing down **non-prod environments** (dev/staging after testing)
> - Cleaning up **failed POC/experiments**
> - Part of **DR drills** — destroy and recreate to validate IaC completeness
> - **NEVER on production** without extreme caution and explicit approval
>
> In our workflow, `destroy` is almost never run manually. Non-prod teardown is automated via CI pipeline (scheduled or triggered)."

---

### Q: What is the purpose of terraform init?

**Project Reference:** P2, P4 (Terraform)

**Answer:**

> "`terraform init` initializes the working directory. Three things it does:
>
> 1. **Downloads providers** — Fetches the AWS provider plugin (or any provider) from the Terraform registry and stores in `.terraform/` directory.
>
> 2. **Configures backend** — Connects to S3 remote state backend. If migrating backends, it offers to copy existing state.
>
> 3. **Downloads modules** — Pulls referenced modules (from Git, registry, or local paths) into `.terraform/modules/`.
>
> **When you MUST re-run init:**
> - Added a new provider or changed provider version
> - Added a new module source
> - Changed backend configuration (e.g., switched S3 bucket)
> - Cloned fresh repo (`.terraform/` is gitignored)
>
> In CI/CD, `terraform init` runs at the start of every pipeline run — ensures clean state, correct providers, and backend connectivity."

---

### Q: Scenario: A bucket was created manually via console. What error do you get when running Terraform plan, and how do you avoid this?
...
> "**Two scenarios:**
>
> **Scenario 1: Terraform has the same bucket in code but it doesn't exist in state**
> - `terraform plan` shows: `Plan: 1 to add`
> - `terraform apply` fails with: `BucketAlreadyExists` or `BucketAlreadyOwnedByYou`
> - Terraform tries to CREATE it, but it already exists in AWS.
>
> **Fix — `terraform import`:**
> ```bash
> terraform import aws_s3_bucket.my_bucket my-bucket-name
> ```
> This adds the existing bucket to state. Next plan shows no changes (if code matches reality).
>
> **Scenario 2: Bucket exists in AWS but NOT in your Terraform code at all**
> - `terraform plan` shows nothing about it — Terraform doesn't know it exists.
> - No error, but it's UNMANAGED infrastructure (drift waiting to happen).
>
> **How to avoid this entirely:**
> 1. **SCPs/IAM policies** — Restrict console S3 creation. Force all infra through Terraform CI pipeline.
> 2. **AWS Config rule** — Detect resources not tagged with `managed-by=terraform`. Alert on unmanaged resources.
> 3. **Team culture** — 'If it's not in Git, it doesn't exist.' Manual creation is a process violation, not a shortcut."

---

### Q: Scenario: If you have to create buckets but need to skip creating servers, how do you achieve this without extensive code changes?

**Project Reference:** P2, P4 (Terraform)

**Answer:**

> "**`-target` flag** — apply only specific resources:
>
> ```bash
> terraform apply -target=aws_s3_bucket.data_bucket -target=aws_s3_bucket.log_bucket
> ```
>
> This tells Terraform: only plan/apply these resources, skip everything else.
>
> **Alternative (better for repeated use): `count` or `feature flags`:**
>
> ```hcl
...
>
> **When to use which:**
> - `-target` → One-time selective apply (quick, ad-hoc). Don't use regularly — partial applies can leave state inconsistent.
> - Feature flag (`count`/`for_each`) → Permanent, repeatable, code-reviewed. Use for components that are conditionally deployed (e.g., DR infra only in prod).
>
> **In practice:** We use feature flags. `-target` is only for emergency hotfixes or initial bootstrapping where you need infra in a specific order."
...
### Q: Why do you need Python if you are using Terraform?

**Project Reference:** P5 (Serverless — Lambda), P7 (Cost Optimization — automation scripts)

**Answer:**

> "Terraform provisions infrastructure. Python handles **logic that Terraform can't:**
>
> 1. **Lambda functions** — Our serverless remediation engine (P5) is Python. EventBridge triggers Lambda which contains the business logic (check violation → determine fix → apply fix → log result). Terraform DEPLOYS the Lambda, Python IS the Lambda.
>
> 2. **Custom automation scripts** — Cost reporting (pull data from AWS Cost Explorer API, generate chargeback reports), tag compliance scanning, resource inventory scripts. These are operational tasks, not infrastructure provisioning.
>
...
# SECTION 23: Terraform Fundamentals

---

### Q: Can you give me an overview of how Terraform works?

**Project Reference:** P2, P4 (Terraform)

**Answer:**

> "Terraform is a **declarative IaC tool** — you define WHAT you want, it figures out HOW to get there.
>
> **Core workflow:**
> 1. **Write** — Define infrastructure in `.tf` files (HCL language). 'I want a VPC, 3 subnets, an EKS cluster.'
> 2. **Init** — `terraform init` downloads providers (AWS plugin) and modules.
> 3. **Plan** — `terraform plan` compares desired state (code) vs current state (state file) → shows what will be created/modified/destroyed.
> 4. **Apply** — `terraform apply` executes the plan → calls AWS APIs to create/modify resources → updates state file.
>
> **The magic is the state file:** Terraform tracks what it has already created. Next time you run plan, it knows 'VPC already exists, but you added a subnet — I'll just create the subnet.'
>
> **Idempotent:** Run apply 10 times — if nothing changed in code, nothing changes in AWS. Safe to re-run."
...
> "Terraform's architecture has 4 key components:
>
> ```
> [.tf Files (HCL)] → [Terraform Core] → [Providers] → [Cloud APIs]
>                          ↕
>                    [State File]
> ```
>
> 1. **Terraform Core** — The engine. Reads `.tf` files, builds a dependency graph (DAG), determines execution order, manages state. This is the `terraform` binary.
>
> 2. **Providers** — Plugins that know how to talk to specific APIs. `hashicorp/aws` provider knows AWS API. `hashicorp/kubernetes` knows K8s API. Each provider translates Terraform resource definitions into API calls.
>
> 3. **State file** — The record of what Terraform has created. Maps HCL resources to real-world resource IDs. Stored remotely (S3) for team collaboration.
>
> 4. **Provisioners** (optional) — Run scripts on created resources (e.g., `remote-exec`). Avoid when possible — prefer user-data or Ansible instead.
>
> **Execution flow:**
> - Core reads code → builds dependency graph (knows VPC must exist before subnet)
> - Compares graph to state → determines changes needed
> - Calls provider plugin → provider makes API calls to AWS
> - Provider returns resource IDs → Core updates state file
>
> **Key design:** Providers are separate plugins. Terraform itself doesn't know anything about AWS — the provider handles that. This is why Terraform works with 3000+ providers (AWS, GCP, Azure, K8s, GitHub, Datadog, etc.)."

---

### Q: What is the disadvantage of a Terraform state file?

**Project Reference:** P4 (Landing Zone — state management)

**Answer:**

> "Main disadvantages:
>
> 1. **Contains sensitive data** — Passwords, secret keys, IPs are stored in plain text in state. If someone gets access to state file → full infrastructure exposure. Mitigation: encrypted S3 bucket, strict IAM policies.
>
> 2. **Single point of failure** — If state is corrupted or lost, Terraform doesn't know what exists. You'd need to `import` every resource manually. Mitigation: S3 versioning (recover previous state), regular backups.
>
> 3. **State locking conflicts** — Two engineers running apply simultaneously → state corruption. Mitigation: DynamoDB lock table (only one apply at a time).
>
> 4. **Drift goes undetected** — State says 'Security Group has 3 rules.' Someone adds a 4th manually. State is stale until next `plan`. Mitigation: scheduled drift detection (`plan` runs nightly).
>
> 5. **Large state = slow operations** — A monolithic state file with 500+ resources makes `plan` take minutes. Mitigation: split state per component/account (we have separate state for networking, compute, database).
>
> **Bottom line:** State is essential (can't work without it) but it needs protection — encryption, versioning, locking, access control, and splitting."

---

### Q: What are some best practices for managing state files?

**Project Reference:** P4 (Landing Zone), P2 (3-Tier)

**Answer:**

> 1. **Remote backend** — S3 + DynamoDB. Never local. Never in Git.
> 2. **Encryption** — S3 server-side encryption (KMS key). State contains secrets.
> 3. **Versioning** — S3 bucket versioning ON. Can recover previous state if corrupted.
> 4. **Locking** — DynamoDB table prevents concurrent applies (race condition protection).
> 5. **Access control** — Only CI pipeline role can write state. Developers get read-only (for `plan` locally if needed).
> 6. **Split state** — Separate state per environment (dev/staging/prod) AND per layer (network/compute/database). One bad apply doesn't destroy everything.
> 7. **No manual state edits** — Use `terraform state mv`, `terraform import`, `terraform state rm`. Never edit JSON directly.
> 8. **Regular backups** — S3 versioning handles this, but also periodic snapshots before risky operations.
> 9. **Drift detection** — Scheduled `terraform plan` in CI (nightly). If plan shows changes when none were made → someone did manual work.
> 10. **State file per team/domain** — Networking team owns network state. App team owns compute state. Clear ownership."
...
### Q: How do you handle joining a company that wants to use Terraform, but they already have 100 EC2 instances created manually?

**Project Reference:** P4 (Landing Zone — adopting IaC for existing infra)

**Answer:**

> "This is a **Terraform adoption/import** project. You don't recreate 100 instances — you bring them under Terraform management.
>
> **Approach (phased):**
>
> 1. **Inventory** — List all 100 instances: instance IDs, types, VPCs, security groups, tags. AWS CLI: `aws ec2 describe-instances --output json > inventory.json`
>
> 2. **Write Terraform code to MATCH existing state** — Don't design ideal state yet. Write `.tf` files that describe what EXISTS. Match instance type, AMI, subnet, SG exactly.
>
> 3. **Import in batches:**
>    ```bash
>    terraform import aws_instance.web_server_1 i-0abc123def456
>    terraform import aws_instance.web_server_2 i-0abc789ghi012
>    ```
>    Or at scale, use **Terraformer** (auto-generates .tf files + imports state from existing infra):
>    ```bash
>    terraformer import aws --resources=ec2_instance --regions=us-east-1
>    ```
>
> 4. **Validate** — Run `terraform plan`. Should show: `No changes. Infrastructure is up-to-date.` If plan shows changes → your code doesn't match reality. Fix code until plan is clean.
>
> 5. **Refactor gradually** — Once imported, start organizing into modules, adding tags, standardizing. Each change goes through normal PR → plan → apply workflow.
>
> **Key principles:**
> - Don't try to import everything in one day. Batch by application/team.
> - Import is non-destructive — it only adds resources to state, doesn't modify them.
> - Start with critical/stable resources. Leave frequently-changing resources for later.
> - Set `lifecycle { prevent_destroy = true }` on imported resources initially — safety net.
>
> **Timeline for 100 instances:** ~2-3 weeks to import and validate. Then ongoing refactoring into modules over months."

---

### Q: What is the impact of changing the Terraform version, and how would you manage that risk?

**Project Reference:** P2, P4 (Terraform CI/CD — version management)

**Answer:**

> "**Impacts of version change:**
>
> 1. **HCL syntax changes** — Terraform 0.12 → 0.13 had major syntax changes. 0.14+ introduced `required_providers` block. Code may not validate.
>
> 2. **State file format** — Newer Terraform may upgrade state file format. Once upgraded, older Terraform can't read it (one-way migration). Critical risk.
>
> 3. **Provider compatibility** — New Terraform version may require newer provider versions. Providers may deprecate/change resources.
>
> 4. **Behavior changes** — Resource lifecycle, plan output, error handling may differ subtly. What worked before might produce different plan.
>
...
>    }
>    ```
>    CI pipeline uses exact version (tfenv or Docker image with pinned version).
>
> 2. **Upgrade process:**
>    - Read changelog/upgrade guide for the version
>    - Upgrade in Dev first → run plan → verify no unexpected changes
>    - If plan is clean → upgrade Staging → then Prod
>    - Never upgrade Terraform and provider simultaneously — one at a time
>
> 3. **State file backup** — Before upgrading: S3 versioning ensures you have the previous state. If upgrade breaks → revert Terraform binary + restore previous state version.
>
> 4. **CI enforces version** — `.terraform-version` file + tfenv in CI. Everyone uses the same version. No 'works on my machine.'
>
> **Rule:** Upgrade Terraform versions deliberately, not accidentally. Test in lower environments first. Treat it like an application upgrade — plan, test, rollback plan."
...
### Q: Terraform plan/apply takes 30 minutes. How can you improve performance?

**Project Reference:** P4 (Landing Zone — large state), P2 (Terraform optimization)

**Answer:**

> "30-minute plan usually means monolithic state file with hundreds of resources. Solutions:
>
> **1. Split state (biggest impact):**
> - Break monolithic config into smaller, independent modules with separate state files:
>   - `networking/` (VPC, subnets, TGW) — own state
>   - `compute/` (EKS, ASG) — own state
>   - `database/` (RDS, ElastiCache) — own state
> - Each `plan` now touches 50 resources instead of 500. Plan time: 2-3 minutes each.
>
> **2. Use `-target` for focused changes (quick fix, not permanent solution):**
> - `terraform plan -target=module.eks` — only plans the EKS module
> - Use sparingly (can miss dependencies)
>
> **3. Parallelism tuning:**
> - `terraform apply -parallelism=20` (default is 10). More parallel API calls = faster.
> - Be careful: too high can hit AWS API rate limits.
>
> **4. Upgrade provider and Terraform version:**
> - Newer AWS provider versions have optimized API calls and caching.
> - Terraform 1.x is significantly faster than 0.12-0.14.
>
> **5. Remove data sources that make unnecessary API calls:**
> - `data \"aws_ami\" \"latest\"` with broad filters → queries ALL AMIs every plan. Pin the AMI ID directly.
>
> **6. Use Terragrunt or Terraform workspaces for modular execution:**
> - Terragrunt runs each module independently — parallel execution across modules.
>
> **Root cause fix:** Architecture problem, not Terraform problem. No single state file should manage 500+ resources. Split by domain/team/environment."

---

### Q: Team X creates EC2 instances, Team B also creates EC2 — there's confusion. How do you handle and simplify this?

**Project Reference:** P4 (Landing Zone — governance), P2 (Terraform modules)

**Answer:**

> "This is a **governance and standardization problem**. Solution: **shared Terraform modules + self-service with guardrails.**
>
> **1. Shared module library (Golden modules):**
> - Platform team (me) creates a standard EC2 module with security baked in:
>   - Encryption enabled by default
>   - Only approved AMIs (hardened, CIS-compliant)
>   - Mandatory tags (owner, team, cost-center, environment)
>   - Private subnet placement (no public instances without exception)
> - Teams X and B CONSUME the module, they don't write raw `aws_instance` resources:
>   ```hcl
>   module \"my_instance\" {
>     source = \"git::https://github.com/org/terraform-modules//ec2?ref=v2.0\"
>     instance_type = \"m5.large\"
>     team         = \"team-x\"
...
>   ```
>
> **2. Separate state per team:**
> - Team X has their own Terraform state/directory
> - Team B has theirs
> - No collision. No overwriting each other's resources.
>
> **3. Enforced via CI/CD:**
> - OPA/Conftest policies in pipeline: reject any PR that uses raw `aws_instance` instead of the approved module
> - SCPs: mandatory tags (untagged → auto-terminated)
>
> **4. Service catalog (optional):**
> - For non-Terraform teams: AWS Service Catalog with pre-approved products (EC2 with specific configs). Click and deploy — within guardrails.
>
> **Result:** Teams get self-service speed. Platform team maintains security/compliance. No confusion about who created what (tags + state isolation)."
