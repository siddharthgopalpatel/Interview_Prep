...
### Q52: How do you ensure this application is highly available?

**Answer:**
HA is achieved at every layer of the architecture:

- **Network layer:** VPC spans 3 AZs. If one AZ goes down, two remain.
- **Web layer:** ALB operates across 3 AZs — one AZ fails, ALB routes to remaining two automatically.
- **Compute layer:** ASG maintains minimum 3 EC2 instances across 3 AZs. Instance dies → ASG replaces in seconds. ALB only sends traffic to healthy instances (health checks every 15s).
- **Database layer:** Aurora Multi-AZ — primary in AZ-a, replica in AZ-b. Failover in <30 seconds. App reconnects automatically via writer endpoint DNS.
- **Network resilience:** NAT Gateway per AZ — one NAT fails, other AZs still have outbound.
- **Security:** SG chaining ensures compromise of one tier can't reach another.

**Global services (Route53, CloudFront, WAF):** Inherently HA — AWS manages across 100+ edge locations. Not my responsibility to make them HA.

**ALB scope:** Regional only — routes within one region. For cross-region HA (DR), Route53 failover sends traffic to a second region (Project 8).

**Result:** Any single component failure — EC2 crash, AZ outage, DB failure — zero downtime for users.

| Failure | Recovery | User Impact |
|---|---|---|
| Single EC2 dies | ASG replaces in 2-3 min, ALB routes to other 2 | None |
| Entire AZ down | ALB + ASG + Aurora failover to remaining AZs | None |
| Aurora primary dies | Replica promoted in <30 sec | ~30 sec blip |
| NAT Gateway fails | Only that AZ loses outbound, others fine | None |
